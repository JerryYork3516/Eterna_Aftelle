...
struct ContinuousProbeMeasurementTests {
    static func main() throws {
        ...
        let audit = ProbePlaybackAudit()
        let old = ProbePlaybackKey(generation: 4, sequence: 1)
        let next = ProbePlaybackKey(generation: 6, sequence: 1)
        audit.associate(runtimeGeneration: 1, playback: old)
        audit.associate(runtimeGeneration: 2, playback: next)
        audit.rendered(old, samples: 240)
        audit.cleared(playbackGeneration: old.generation)
        audit.rendered(next, samples: 240)
        audit.completion(old, accepted: false)
        audit.injectedInput(interrupting: 1, at: 100)
        let decision = UUID()
        audit.confirmed(decision, interrupting: 1, at: 200)
        audit.confirmed(decision, interrupting: 1, at: 200)
        try check(audit.snapshot["stale_playbacks"] == 0, "current_playback_misclassified")
        try check(audit.snapshot["stale_callbacks_rejected"] == 1, "rejection_not_observed")
        try check(audit.snapshot["confirmed_decisions"] == 1, "decision_poll_counted_twice")
        try check(audit.snapshot["unmatched_interruptions"] == 0, "injected_user_misclassified")
        // Negative controls: observed defects must remain visible, never be filtered.
        audit.rendered(old, samples: 13)
        let acceptedOld = ProbePlaybackKey(generation: 4, sequence: 2)
        audit.associate(runtimeGeneration: 1, playback: acceptedOld)
        audit.completion(acceptedOld, accepted: true)
        audit.confirmed(UUID(), interrupting: 1, at: 300)
        audit.confirmed(UUID(), interrupting: 2, at: 400)
        audit.rendered(nil, samples: 7)
        try check(audit.snapshot["stale_playbacks"] == 1, "old_audio_false_pass")
        try check(audit.snapshot["stale_rendered_samples"] == 13, "old_sample_count")
        try check(audit.snapshot["stale_callbacks_accepted"] == 1, "accepted_stale_callback_false_pass")
        try check(audit.snapshot["unmatched_interruptions"] == 2, "unprompted_or_duplicate_interrupt_false_pass")
        try check(audit.snapshot["unknown_observations"] == 1, "missing_provenance_false_pass")
        let delayed = ProbePlaybackAudit()
        delayed.associate(runtimeGeneration: 1, playback: old)
        delayed.completion(old, accepted: true)
        delayed.deliveredRetiredCallback(old)
        try check(delayed.snapshot["stale_callbacks_accepted"] == 1, "late_delivery_observation_false_pass")
        try check(delayed.snapshot["retired_callbacks_missing_disposition"] == 0, "late_delivery_disposition_missing")
        delayed.associate(runtimeGeneration: 2, playback: next)
        delayed.deliveredRetiredCallback(next)
        try check(delayed.snapshot["retired_callbacks_missing_disposition"] == 1, "unsettled_callback_false_pass")
        delayed.completion(next, accepted: false)
        try check(delayed.snapshot["retired_callbacks_missing_disposition"] == 0, "settled_callback_missing")
