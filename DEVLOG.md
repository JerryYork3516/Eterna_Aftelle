...
- 2026-08-16 — Stage 7.5.11-A6 自动化收口，等待真机 Human Gate — A5 正式链不再在首个 ASR final 后停止采集；播放期间继续消费既有 AEC3 source gate 的 near-end / double-talk 证据。确认近端讲话后，Host 只负责立即 pre-clear 既有 Playback，RuntimeCore 原子失效旧 generation、取消旧 ASR / TTS 与未完成 LLM 结果，再创建新 ASR generation；旧 canonical response 不能提交 Session / Memory / Dialogue History。resident-only、residual echo、alignment 丢失与 route rebuild 继续由既有 source gate / safe half-duplex fallback 抑制，USB / Bluetooth 路由恢复仍需真机 Human Gate，不以自动化代替声学验收。未改 AEC3 48 kHz / 10 ms 域、Provider 架构、Runtime API、DR / Store schema 或平台 target。
