...
- **现在在做**:R8.3.3 User Barge-in Latency & Stale Audio Closure — PASS / FROZEN
- **上一步刚完成**:正式插话链已冻结本地 latency，并证明 generation N 的旧音频、旧输出事件与旧 Playback callback 在 confirmed 后全部 fail closed，N+1 仍可正常 Input / Output / Playback
- **当前卡在**:无
- **下一步**:只允许进入 R8.4 Double-talk / Turn-taking / Backchannel；真机与 live Qwen 仍走 Human Gate
- **本轮范围**:R0–R8.3.3 已冻结；不回头改冻结事实，不提前实现 R8.4、R8.5、R9 或 R10
