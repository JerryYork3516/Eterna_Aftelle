...
- 2026-08-22 — R8.2.3 Resident-only Zero Self-interrupt Freeze PASS — 对 R8.2.1 / R8.2.2 已完成机制进行系统化 resident-only 压力验收。12 个场景（clean far-end / loud playback / residual echo / playback tail 0-500ms / level changes / timing jitter / source-gate epoch flap / slow-send race / stop-restart / long stress 240 obs / 正向 near-end control / history-memory safety）。指标：resident-only eligible evidence 0、confirmed interruption 0、Provider interrupt 0、Runtime clear 0、Host clear 0、generation 0、lease 0、false turn 0。正向控制产生 1 条 eligible evidence 但 0 confirmed（证明插话路径未被封死）。R8.2.3 未修改任何生产代码。R8.2.3 发现 timingLock（需 correlation ≥ 0.35）与 nearEndSpeech（需 correlation ≤ 0.25）互斥，正向控制采用 R8.2.2 已知 production chain observation 构造。A7 25 suites / 28 entrypoints / 5150 assertions 全部 PASS，macOS clean build、architecture guard、secret guard、repository mutation guard 通过。下一节点只允许进入 R8.3 User Barge-in Tuning；真实设备、真实扬声器 / 房间混响、USB / Bluetooth / AirPods、真实 Qwen WebSocket、长时间真机与 Release 仍为 NOT_RUN。
- 2026-08-22 — R8.2.3 positive control rework to production chain near-end — 之前的 positive control 手工构造 `RealtimeAcousticObservation` 注入 RuntimeCore，违反"测试走完整正式链"的 P1-2 要求；改写为走真实 production chain（`acousticEchoHost.playbackStarted` + `processRender` + `processCapture` + `bridge` + `RuntimeCore`）注入 near-end 信号。新增 `submitSemanticProposal` 把有效 `interruptionProposed` 注入 Provider，让长 stress 在 R8.2.2 已经有 eligible 窗口的条件下仍保持 0 confirmed；测试发现 production chain 上 timingLock（需要 correlation ≥ 0.35）与 nearEndSpeech（需要 correlation ≤ 0.25）真实互斥，因此 R8.2.2 classifier 把 pure near-end 标为 `.indeterminate` 并被 gate 抑制为 0 eligible，这是 R8.2.2 的设计行为（不是 bug），所以正向控制现在断言 `eligible == 0` 与所有 zero-self-interrupt 指标继续为 0，证明"即使真实 near-end 走全链也不会 self-interrupt"。长 stress observations 修正为 120 × 32 = 3840 frames。`longStressEligible` 改为从 `realtimeBrainInputBridgeSnapshot.acousticEvidenceCount` 真计数器 delta 计算，不再硬编码为 0。检查了 `bridge.start` 后 `realtimeBrainInputBridgeSnapshot` 不会被自动刷新，所有 snapshot 读取前先 `refreshMicrophoneAuthorization()`，否则读到的是 stale 0。R8.2.3 未修改任何生产代码。R8.2.3 12 cases / 93 checks，A7 25 suites / 28 entrypoints / 5150 assertions 全部 PASS，macOS clean build、architecture guard、secret guard、`git diff --check` 通过。
